...
- [x] Real Rust engine change exists (else 50% cap). (`SpeedrunService` in the `anki` crate: 6 RPCs called from Python — `speedrun_ping`, `compute_readiness`, `get_mastery_state`, `get_mastery_ordered_new_cards`, `get_points_at_stake_order`, `get_study_plan` — plus two opt-in, default-off live-queue reorders in `build_queues`: `speedrunMasteryScheduler` (new-card tier order) and `speedrunPointsAtStake` (review order).)
