...
Performance, and Readiness, never blended into one number."_
...
`SpeedrunService` in the `anki` crate with `compute_readiness`, `get_mastery_state`,
`get_mastery_ordered_new_cards`, and `get_points_at_stake_order`, plus two opt-in
live-queue reorders in `build_queues`, wired proto → Rust → Python → web. It's not
a wrapper and not a JS reimplementation: a JS/Swift rewrite would cap the grade."_
...
≥50% weighted coverage. That's a **Rust assertion**, not a UI hint. The
...
tests, **all three signal cards now show a point AND a range** side by side, so
point the camera at the whole row for shot 4: Memory **90% (85-96%)** (mean FSRS
retrievability with a 10th-90th percentile band), **Performance** its pooled
graded practice-test accuracy with a **95% Wilson band** (measured per subtopic;
abstains below the sample floor), and Readiness projected **5.5 (4.6-6.4), P(pass)
14%**, confidence 0.91, next action "focus multivariate." Say: _"This is a
**synthetic demo persona**, labelled as such and reproducible from a seed. It's
...
count, because the review loop is running on the real Rust scheduler.
...
test: `make paraphrase` → _"recall 73% vs reworded 32%, a +41-point gap. That's
proof Performance isn't just Memory in disguise; a copycat control collapses to zero."_
...
test items, a one-command benchmark, and a crash test with zero corruption, so
...
desktop, or sync once that's wired) and run a review, which exercises the same
...
- **Throwaway macOS VM (most airtight; needs ~40 GB free, but currently only ~18 GB).**
