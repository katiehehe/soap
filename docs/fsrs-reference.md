...
> FSRS version differs from what's here, update this file — formulas change between versions.
...
- **Same-day memory is weak.** FSRS has only a crude heuristic for same-day / short-term
  memory — no real short-term model. Trust its predictions least for cramming behavior
