...
- **Readiness** returns nothing below **≥ 200 graded reviews AND ≥ 50% weighted
  practiced coverage**, and even once those pass a readiness NUMBER additionally
  needs **≥ 30 graded practice-test questions** (see §3); short of that it still
  abstains. This is a Rust assertion, not a UI hint: `compute_readiness` returns
  `oneof { NoScore, ReadinessScore }` (`rslib/src/speedrun/service.rs`), so a bare
  readiness number literally cannot be emitted below threshold.
...
- **Displayed range:** the Memory signal is shown as the **mean** per-card FSRS
  retrievability (point) with a **10th-90th percentile band** across reviewed
  syllabus cards (`memory_recall` in `rslib/src/speedrun/mastery.rs`), so it
  carries a measured range, not a bare point. It abstains ("Not yet scored")
  until at least one card has been reviewed.
...
- **Displayed range:** on the dashboard the Performance signal is the **pooled
  graded practice-test accuracy** (correct / questions across measured subtopics)
  shown with a **95% Wilson band** (`wilsonInterval`, the same band math readiness
  uses), so it is never a bare point. Below the per-subtopic sample floor
  (`MIN_PERF_QUESTIONS`) it abstains ("Not yet measured") rather than report a
  proportion from one or two answers.
...
- **Mapping (fixed in advance, in `readiness_from_practice`):** let p̂ = correct /
  ...
  - **P(pass):** SOA P passes at scaled ≥ 6, i.e. p ≥ **0.60** under the linear
    map. P(pass) = Φ((p̂ − 0.60) / se), the normal approximation to the binomial <!-- dash-ok -->
    proportion (se = √(p̂(1−p̂)/n)). 0.60 is a stated assumption, recalibratable <!-- dash-ok -->
...
- **Commands:** `make calibration` (memory); performance/readiness evals land
  with their datasets. All deterministic, so anyone can re-run and get the same
  numbers.
