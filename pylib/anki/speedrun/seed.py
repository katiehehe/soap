...
SEED_CARDS: list[SeedCard] = [
    ...
    SeedCard(
        ...
        "discrete_dists",
        "medium",
        "For X ~ Poisson(lambda), give Var(X).",
        "Var(X) = lambda.",
    ),
    SeedCard(
        "univariate",
    ...
    SeedCard(
        ...
        "continuous_dists",
        "easy",
        "For X ~ Exponential with rate lambda, give E[X].",
        "E[X] = 1 / lambda.",
    ),
    SeedCard(
        "univariate",
        "continuous_dists",
        "easy",
        "For X ~ Exponential with rate lambda, give Var(X).",
        "Var(X) = 1 / lambda^2.",
    ),
    SeedCard(
        "univariate",
    ...
    SeedCard(
        ...
        "sets_axioms",
        "medium",
        "State inclusion-exclusion for three events P(A or B or C).",
    ...
    SeedCard(
        ...
        "combinatorics",
        "easy",
        "Give the permutation count of k items from n (order matters).",
        "nPk = n! / (n - k)!.",
    ),
    SeedCard(
        "general",
        "combinatorics",
        "easy",
        "Give the combination count of k items from n (order does not matter).",
        "nCk = C(n, k) = n! / (k! (n - k)!).",
    ),
    SeedCard(
        "general",
        "combinatorics",
        "medium",
        "Give the multinomial count of arrangements of n items in groups of "
        "sizes n1, ..., nk.",
        "n! / (n1! n2! ... nk!).",
    ),
    SeedCard(
        ...
        "A and B are independent iff P(A | B) = P(A) (equivalently P(B | A) = P(B)).",
    ),
    SeedCard(
        "general",
        "independence",
        ...
        "Yes. Independence is preserved under complements (A & B', A' & B, A' & B').",
    ),
    SeedCard(
        "general",
        "add_mult_rules",
        "easy",
        "For mutually exclusive A and B, give P(A or B).",
    ...
    SeedCard(
        ...
        "conditional",
        "medium",
        "State the law of total probability for a partition A1, ..., An.",
    ...
    SeedCard(
        ...
        "bayes",
        "medium",
        "State Bayes' theorem with a partition A1, ..., An (denominator expanded).",
    ...
    SeedCard(
        ...
        "rv_basics",
        "easy",
        "Give the two properties of a probability mass function p(x).",
        "0 <= p(x) <= 1 for all x, and sum of p(x) over the support = 1.",
    ),
    SeedCard(
        "univariate",
        "rv_basics",
        "easy",
        "Give the two properties of a probability density function f(x).",
        "f(x) >= 0 for all x, and the integral of f(x) over the support = 1.",
    ),
    SeedCard(
        "univariate",
        "rv_basics",
        "medium",
        "List the defining properties of a CDF F(x).",
        "Non-decreasing, right-continuous, F(-infinity) = 0, F(+infinity) = 1.",
    ),
    SeedCard(
        "univariate",
        "rv_basics",
    ...
    SeedCard(
        ...
        "rv_basics",
        "easy",
        "For a continuous X, give P(a < X <= b) from the CDF.",
        "P(a < X <= b) = F(b) - F(a).",
    ),
    SeedCard(
        "univariate",
        "expectation",
        "easy",
        "Define E[X] for a discrete X with pmf p.",
        "E[X] = sum of x p(x) over the support.",
    ),
    SeedCard(
        "univariate",
        "expectation",
        ...
        "Continuous: integral of g(x) f(x) dx. Discrete: sum of g(x) p(x).",
    ),
    SeedCard(
        "univariate",
        "expectation",
        "easy",
        "Give E[aX + b] (linearity of expectation).",
        "E[aX + b] = a E[X] + b.",
    ),
    SeedCard(
        "univariate",
        "expectation",
        "medium",
        "For a non-negative X, give E[X] via the survival function.",
        "E[X] = integral from 0 to infinity of S(x) dx = integral of [1 - F(x)] dx.",
    ),
    SeedCard(
        "univariate",
        "expectation",
    ...
    SeedCard(
        ...
        "expectation",
        "medium",
        "How do you get moments from the MGF M_X(t)?",
        "M'(0) = E[X], M''(0) = E[X^2], and the k-th derivative at 0 is E[X^k].",
    ),
    SeedCard(
        "univariate",
        "variance",
    ...
    SeedCard(
        ...
        "easy",
        "Give Var(aX + b).",
        "Var(aX + b) = a^2 Var(X) (the constant b does not affect variance).",
    ),
    ...
    SeedCard(
        ...
        "discrete_dists",
        "easy",
        "For X ~ Bernoulli(p), give the pmf.",
        "p(1) = p, p(0) = 1 - p.",
    ),
    SeedCard(
        "univariate",
    ...
    SeedCard(
        ...
        "discrete_dists",
        "medium",
        "For X ~ Binomial(n, p), give the pmf p(k).",
        "p(k) = C(n, k) p^k (1 - p)^(n - k), for k = 0, 1, ..., n.",
    ),
    SeedCard(
        "univariate",
        "discrete_dists",
        "medium",
        "For X ~ Geometric(p) (trials until the first success, k = 1, 2, ...), "
        "give the pmf.",
        "p(k) = (1 - p)^(k - 1) p.",
    ),
    SeedCard(
    ...
    SeedCard(
        ...
        "discrete_dists",
        "medium",
        "For X ~ Negative Binomial(r, p) (trials until the r-th success), give "
        "E[X].",
        "E[X] = r / p.",
    ),
    SeedCard(
    ...
    SeedCard(
        ...
        "discrete_dists",
        "hard",
        "For X ~ Hypergeometric (n draws without replacement from N items, K "
        "successes), give E[X].",
        "E[X] = n K / N.",
    ),
    SeedCard(
        ...
        "hard",
        "For X ~ Hypergeometric (n draws without replacement from N items, K "
        "successes), give Var(X).",
        "Var(X) = n (K/N) (1 - K/N) (N - n)/(N - 1) (finite-population correction).",
    ),
    SeedCard(
        "univariate",
        "discrete_dists",
        "medium",
        "For X ~ Poisson(lambda), give the pmf p(k).",
        "p(k) = e^(-lambda) lambda^k / k!, for k = 0, 1, 2, ...",
    ),
    SeedCard(
        "univariate",
        "discrete_dists",
        "medium",
        "For X discrete uniform on {1, ..., n}, give E[X].",
        "E[X] = (n + 1) / 2.",
    ),
    SeedCard(
        "univariate",
        "discrete_dists",
        "medium",
        "For X discrete uniform on {1, ..., n}, give Var(X).",
        "Var(X) = (n^2 - 1) / 12.",
    ),
